...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card p-4 shadow-sm border-0 my-3">
            <div class="mb-4">
                <h3 class="fw-bold text-primary-color mb-1">新規会員を直接追加</h3>
                <p class="text-muted">システム管理者が直接新規会員を登録します。登録された会員は即座にステータスが「有効（正式在籍）」となり、当年度の在籍レコードが自動生成され、パスキー設定用のワンタイムURLが発行されます。</p>
            </div>
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="needs-validation" novalidate>
                ...
                <h5 class="fw-bold text-dark border-bottom pb-2 mb-3">4. 初期権限（ロール設定） <span class="text-danger">*</span></h5>

                <div class="mb-4">
                    <div class="d-flex gap-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="roles[]" value="general" id="role-general" {{ is_array(old('roles', ['general'])) && in_array('general', old('roles', ['general'])) ? 'checked' : '' }}>
                            <label class="form-check-label small" for="role-general">一般会員</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="roles[]" value="kanji" id="role-kanji" {{ is_array(old('roles')) && in_array('kanji', old('roles')) ? 'checked' : '' }}>
                            <label class="form-check-label small text-warning-emphasis fw-semibold" for="role-kanji">幹事</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="roles[]" value="admin" id="role-admin" {{ is_array(old('roles')) && in_array('admin', old('roles')) ? 'checked' : '' }}>
                            <label class="form-check-label small text-danger fw-semibold" for="role-admin">システム管理</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="roles[]" value="equipment_manager" id="role-equipment" {{ is_array(old('roles')) && in_array('equipment_manager', old('roles')) ? 'checked' : '' }}>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary px-5">ユーザーを登録</button>
                </div>
            </form>
        </div>
    </div>
