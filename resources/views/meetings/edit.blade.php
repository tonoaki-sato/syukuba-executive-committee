    ...
    <div class="col-md-8">
        <div class="card p-4 shadow-sm border-0 my-3">
            <div class="mb-4 border-bottom pb-2">
                <h3 class="fw-bold text-primary-color mb-1">会議情報の編集</h3>
                <p class="text-muted small">登録済みの会議予定を修正します。※会議タイプの変更は、既存のアサインメンバーには影響しません。</p>
            </div>
            ...
            <form action="{{ route('meetings.update', $meeting) }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="fiscal_year" id="fiscal_year" class="form-control border-secondary-subtle" 
                           value="{{ old('fiscal_year', $meeting->fiscal_year) }}" min="2000" max="2100" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="type" id="type" class="form-select border-secondary-subtle" required>
                        <option value="board" {{ old('type', $meeting->type) == 'board' ? 'selected' : '' }}>幹事会（幹事のみ自動アサイン）</option>
                        <option value="general" {{ old('type', $meeting->type) == 'general' ? 'selected' : '' }}>総会（全正式会員を自動アサイン）</option>
                        <option value="subcommittee" {{ old('type', $meeting->type) == 'subcommittee' ? 'selected' : '' }}>部会（全正式会員を自動アサイン）</option>
                        <option value="work" {{ old('type', $meeting->type) == 'work' ? 'selected' : '' }}>作業（全正式会員を自動アサイン）</option>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control border-secondary-subtle" 
                           value="{{ old('name', $meeting->name) }}" placeholder="例：第4回 ござ市部会、7月度定例幹事会" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="datetime-local" name="held_at" id="held_at" class="form-control border-secondary-subtle" 
                           value="{{ old('held_at', $meeting->held_at ? $meeting->held_at->format('Y-m-d\TH:i') : '') }}" required>
                </div>
